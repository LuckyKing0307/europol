...
return [
     ...
     */
    '1cRouteNameCatalog' => 'exchange/commerceml',
    ...
    /*
     * Модель, которая будет формировать XML для отправки информации о
     ...
     * пройти, перед тем, как запрос попадет в контроллер пакета. Если указывается
     * группа middleware, то в ней обязательно быть middleware, которая содержит в себе
     * инициализацию сессии (см. в документации раздел о стандартных группах middleware).
     * Без инициализации сессии ничего работать не будет. При этом ВАЖНО - не
     * должно быть посредника \App\Http\Middleware\EncryptCookies::class (входит в состав
     ...
     * урл запроса от 1С (параметр logCommandsFullUrl), соответственно - отпадает
     ...
     * в отличие от стандартного обмена, требует передачи дополнительных
     * параметров - CSRF, номер версии XMl
     */
    'isBitrixOn1C'       => false,

    /*
     * Версия протокола обмена, требуется только для случаев, если в 1С
